...
- 🚀 **即点即用**：访客打开页面即可添加 2FA、看到验证码，纯本地不上传
- 🔐 **端到端加密同步**：每个项目用 Sync Secret 派生 AES-GCM 密钥，云端只存密文
- 🛡 **管理员模式**：通过 `ADMIN_KEY` 一键解锁同步、云端浏览、密钥托管、批量迁移等高级能力
- 👻 **strict / open 双模式**：strict 模式下普通访客根本拿不到云端密文
- 📱 **PWA + 离线**：装到手机主屏，断网也能生成验证码
- 🎨 **现代化 UI**：玻璃拟态 + 弹簧动效 + 暗色主题，移动端友好
...
### 1. 普通访客「即点即用」
打开页面 → 「+」添加账户 → 粘贴密钥 → 立刻显示验证码。

完全本地（localStorage），无需登录、无需创建任何"项目"，关闭浏览器后数据仍在。点击卡片即复制验证码，长按弹出更多操作（分享 / 删除 / 复制 otpauth）。

### 2. 自用多设备同步
设置入口（右上齿轮）→「项目」→「新建项目」，填项目名、Sync ID、Sync Secret 三项即可创建端到端加密的同步空间：
...
### 3. 管理员高阶运维
进入「关于」→「输入 Admin Key 登录」后，"管理员"标签页自动出现：

- 列出云端所有同步项目（KV `sync:*`）
- 一键批量解密（多个 Secret 并发尝试）
...
| 隐藏交互 | 三击标题设 Server Token | 抽屉「关于 → 输入 Admin Key 登录」可见入口 |
...
1. 部署 + 配置 KV
2. 设置环境变量 `ADMIN_KEY`
3. 在前端"关于"页输入 Admin Key 登录管理员模式
